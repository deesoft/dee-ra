...
class GoodsMovement extends \yii\db\ActiveRecord
{
    ...
    public function postGL($factor)
    {
         ...
         * Header Journal
         */
        $model_journal = new \app\models\accounting\GlHeader;
        $model_journal->periode_id = \app\models\accounting\AccPeriode::find()->active()->one()->id;
        $model_journal->date = date('Y-m-d');
        $model_journal->status = \app\models\accounting\GlHeader::STATUS_RELEASED;
        $model_journal->reff_type = \app\models\accounting\GlHeader::REFF_GOODS_MOVEMENT;
        $model_journal->reff_id = $this->id;
        $model_journal->branch_id = (isset(Yii::$app->profile->branch_id)) ? Yii::$app->profile->branch_id : -1;

        $esheet = ($factor == 1) ? \app\models\accounting\EntriSheet::find()->where('code=:dcode', [':dcode' => 'ES001'])->one()
                : \app\models\accounting\EntriSheet::find()->where('code=:dcode', [':dcode' => 'ES003'])->one();
        ...
        $ndtl = new \app\models\accounting\GlDetail();
        $ndtl->coa_id = $esheet->d_coa_id;
        $ndtl->header_id = null;
        $ndtl->amount = $this->totalValue;
        $newDtls[] = $ndtl;

        $ndtl1 = new \app\models\accounting\GlDetail();
